      ...
      Facility:''
              ...
              if (action !== TimePickerAndroid.dismissedAction) {
                ...
                if (sum < hour * 60 + minute) {
                  ...
                  [
                    ...
                    {
                      text: 'Yes' ,onPress:()=>{this.extend(upto)
           ...
           fun(flag,upto,from){
            ...
            firebase.database().ref('/Booking/'+this.state.Date+"/"+100+"/"+this.state.Ind+"/").update({
                ...
                Alert.alert("Extension Failed !!","Can't Extend upto "+upto+
            ...
            extend=(upto)=>{
              var flag=0;
              var from;
             var ref = firebase.database().ref('Booking/'+this.state.Date+'/'+100+"/");
             ref.orderByChild("To").once('value',(snapshot) => {
               snapshot.forEach((snap) => {
                
                 var to=(snap.val().To.toString().replace(":","."));
                 from=(snap.val().From.toString().replace(":","."));
       ...
       NetCon();
         ...
         ref.once('value').then(function(snapshot){
          ...
          if(!snapshot.exists())
            ...
            var from =(snapshot.val() && snapshot.val().From);
        var to = (snapshot.val() && snapshot.val().To);
